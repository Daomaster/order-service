...
func CreateOrder(src []string, des []string) (*Order, error) {
	...
	d, err := distance.Calculate(src, des)
	if err != nil {
		return nil, err
	}
...
func TakeOrder(id int) error {
	...
	if o.Status == StatusTaken {
		return res_error.ErrOrderAlreadyTaken
	}
...
func GetOrders(page int, limit int) ([]*Order, error) {
	...
	err := db.Offset(page).Limit(limit).Find(&orders).Error
